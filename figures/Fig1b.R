...
{
       SL <- c("sl_random_oblique") 
       {M <- Inf; il <- il_infinite_sites;  sl <- sl_random_oblique;      rec <- record_inf_trait_pop_large}
 
parameters <- expand.grid(n = 500,        # Population size, N # USER INPUT
                          g = 5e2L,       # Number of generations # USER INPUT
...
pl<-lapply(1:length(res), function(i){
    g <- length(res[[i]]$record$c_pop)
    net <- res[[i]]$parameters$p_r
    if(net==-1) {
        res[[i]]$record$adjm <- matrix(1,ncol=res[[i]]$parameters$n,nrow=res[[i]]$parameters$n)
        diag(res[[i]]$record$adjm) <- 0
        }
    TIME<-1:g
    
    # Histogram
    sim_at_mean_s <- which(res[[i]]$record$c_pop==round(mean(res[[i]]$record$c_pop)) & (1:g)>=(g/2)) |> mySample(1)
    if(length(sim_at_mean_s)==0) sim_at_mean_s <- sample((1:g)>=(g/2), 1)

    rep_all<-as.data.frame(res[[i]]$record$repertoire)
    rep_all <- apply(rep_all, 2, as.numeric)
    n<-ncol(rep_all)
    colnames(rep_all)<-1:n #c((floor(n/2)+1):n, 1:floor(n/2))
    
    prop_same_neibs_tt <- do.call("rbind", lapply(sim_at_mean_s, function(tt){
      rep_at_t <- rep_all[tt, ]
      #sort(table(rep_at_t), decreasing = T)
      prop_same_neibs <- mapply(1:length(rep_at_t), FUN = function(ind){
        neibs <- which(res[[i]]$record$adjm[ind, ]==1)
        sum(rep_at_t[neibs] == rep_at_t[ind]) / length(neibs)
      })
      return(prop_same_neibs)
    }))
    prop_same_neibs_tt <- as.data.frame(prop_same_neibs_tt)
...
plot_grid(pl[[1]],pl[[2]],pl[[3]],pl[[4]],pl[[5]], nrow=1, labels="b")
